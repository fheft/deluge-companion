...
<div class="bg-neutral-800 p-4 rounded-lg border border-neutral-700">
  <span
    class="bg-neutral-300 font-normal text-neutral-800 rounded-full px-2 inline-block text-xs whitespace-nowrap"
  >
    {shortcut.category}
  </span>
  ...
  <p class="flex gap-x-2 gap-y-4 flex-wrap" title={shortcut.command}>
    {#each shortcut.commands as command}
      <div
        class="rounded-md shadow-sm border border-neutral-400 gap-x-2 gap-y-0 px-4 py-2 grid bg-neutral-700 command items-baseline justify-items-center"
      >
        <ActionView {command} />
        <TargetView {command} />
      </div>
